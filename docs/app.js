...
function simulate(P, PMT, rPct, years, n = 12) {
  const r = rPct / 100,
    periods = Math.max(1, Math.floor(years * n)),
    periodRate = r / n,
    periodContribution = (PMT * 12) / n;
  let bal = P,
    contrib = 0,
    rows = [];
  for (let k = 1; k <= periods; k++) {
    bal = bal * (1 + periodRate) + periodContribution;
    contrib += periodContribution;
    if (k % n === 0)
      rows.push({
        year: k / n,
        balance: bal,
...
function drawChart(c, { labels, total, principal }) {
  ...
  labels.forEach((year, i) => {
    if (year % 2 === 0) ctx.fillText(`${year}y`, xS(i), H - pad.b + 18);
  });
  ctx.textAlign = "left";
...
function update() {
  ...
  byId("principalStat").textContent = fmt(P);
  ...
  byId("tContribution").textContent = fmt(last.contribution);
  byId("tInt").textContent = fmt(last.interest);

  const labels = [0, ...rows.map((r) => r.year)],
    total = [P, ...rows.map((r) => r.balance)],
    principal = [P, ...rows.map((r) => P + r.contribution)];
  ...
  [
    ...
    byId(id).textContent = fmt(simulate(P, PMT, r, yrs, n).at(-1).balance);
  });
}

const years = byId("years"),
  yearsRange = byId("yearsRange"),
  yearsLabel = byId("yearsLabel");
const syncYears = (value) => {
  years.value = value;
  yearsRange.value = value;
  yearsLabel.textContent = value;
  update();
};
years.addEventListener("input", () => syncYears(years.value));
yearsRange.addEventListener("input", () => syncYears(yearsRange.value));

["principal", "monthly", "rate", "freq"].forEach((id) =>
...
document.querySelectorAll("[data-preset]").forEach((btn) =>
  btn.addEventListener("click", () => {
    ...
    yearsLabel.textContent = years.value;
    update();
  }),
);
